...
import type { BrandLine, Visual } from '@shared/domain/brand';
import { PALETTE_PRESETS } from '@shared/domain/catalog';
import { SINGLE_TEMPLATES, TEMPLATES, brandKit, emptyCardText, type Aspect, type CardText, type TemplateId } from '@shared/domain/visual';
import { templateFallback } from '@shared/domain/line';
...
].filter((job: { templateId: TemplateId; custom?: string }) => only.size === 0 || only.has(job.custom ?? job.templateId));
